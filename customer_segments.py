...
# **回答:**
# - 0: 可能是咖啡厅，Milk、Grocery量比较大，符合咖啡厅的特性
# - 1: 可能是咖啡厅，Milk、Grocery量比较大，Detergents Paper量也较大，相比 samples 0 咖啡厅概率更大
# - 2: 可能是零售商，Fresh的数据75%以上，Frozen的数据也在75%以上，其他数据不是很多，符合零售商的特性
...
# **回答:**
#  - 尝试预测：Detergents_Paper 
#  - 预测的得分：0.728655181254
#  - 这个特征对于区分用户的消费习惯来说：有必要。因为预测分数较高，通过其他特征拟合出来的可能性较高。
...
# **回答:**
# - 存在一些特征他们彼此之间存在一定程度的相关性 。如，grocery 和 Detergents_paper，Frozen 和 Delicatessen
# - 这个结构验证了我尝试预测的特征的相关性 
# - 这个特征的数据非正态分布，大多数的数据点分布集中在较小的范围区间
...
total_outliers = np.array([], dtype='int64')
...
for feature in log_data.keys():
    ...
    total_outliers = np.append(total_outliers, outlier_points.index.values.astype('int64'))
...
total_outliers, index = np.unique(total_outliers, return_inverse=True)
# Count number of occurrences of each value in array of non-negative ints.
x = np.bincount(index)
outliers = total_outliers[x >= 2]
...
# 
# 以上这些数据在多于一个特征下被看作了异常点，应该被移除，因为这些异常值都是比较低的数字，考虑到这些异常值会影响到其他特征数据，导致失真。考虑到这些异常值的数据集规模，删除不会造成明显的影响，由此删除是比较合理的。
...
# - 第一个维度，Milk、Grocery、Detergents_Paper、Delicatessen 权重特征为负，彼此之间存在关联性，可能是咖啡厅
# - 第二个维度，Fresh、Frozen、Delicatessen 权重特征为负，彼此之间存在关联性，可能是零售商
# - 第三个维度，Frozen、Delicatessen 权重特征为正，彼此之间存在关联性，可能是零售商
# - 第四个维度，Fresh、Delicatessen 权重特征为负，彼此之间存在关联性，可能是零售商
...
# **回答:**
# - 使用 K-Means 聚类算法的优点是原理简单、实现起来也比较简单，收敛速度快，聚类效果较优，算法的可解释度比较强，主要需要调参的参数仅仅是簇数k。 
# - 使用高斯混合模型聚类算法的优点是，是一种业界广泛使用的聚类算法，该方法使用了高斯分布作为参数模型，并使用了期望最大（Expectation Maximization，简称EM）算法进行训练。
# - 选用了这两个算法中的高斯混合模型聚类算法，因为处理过的数据基本呈正态分布，由此可能会达到较好的预期。
...
score_result.columns.name = 'Clusters'

for i in range(2, 8):
    score = predictGMM(i) 
    score_result = score_result.append(pd.DataFrame([score], columns=['Score'], index=[i]))
...
plt_data = data.append(true_centers)

# 计算沿着轴的数值数据，等值的排名是这些值的排名的平均值，然后进行筛选，再换算成百分比
t_result = plt_data.rank(axis=0, pct=True).round(decimals=3).loc[['Segment 0', 'Segment 1']] * 100
print (t_result)


# **回答:**
# - Cluster 0，可能是零售商，Segment 0 的各项特征值基本在30%和60%区间，比较符合零售类商店的特征，啥都卖，没有啥卖的特好，比较均衡；
# - Cluster 1，可能是咖啡厅，Segment 1 的值 Milk、Grocery 和 Detergents Paper 超过75%，高于平均值，但是其他的特征数量较少，大体符合咖啡店主要日常消耗品的特性。
...
# **回答:**
# - Sample point 0 与 Sample point 1 是咖啡厅， Sample point 2 是零售商
# - 之前对样本的预测和现在的结果基本符合
...
# **回答：**
# 
# - 小范围试错：在每个分类中找出相等的小部分客户，进行试点测试，观察派送从一周五天降到一周三天有何反馈；
# - 如果业务还是比较稳定，且未收到客户投诉或建议，说明可以执行新派送策略
# - 如果收到大量反馈或投诉意见，那么说明不适合新的派送策略，建议改回原来的派送策略
...
# **回答：**
# - 存在聚类特征'cluster'，分数更高，预测可能会更加准确
# - 启发是：非监督学习与监督学习可以相互促进，提高准确率
...
# **回答：**
# - 选择的聚类算法和聚类点的数目是两个，与内在的旅馆/餐馆/咖啡店和零售商的分布相比，有足够好，在Dimension 1 的值为 0 的纵轴将二者分开
# - 这个分类和前面我对于用户分类的定义基本一致
